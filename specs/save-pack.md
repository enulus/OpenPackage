...
- **Copy the entire package** from the package root into the local registry under the computed WIP version:
  - **Root package**: copy from `<cwd>/` (universal subdirs, root files, `root/` direct copy, etc.)
  - **Nested package**: copy from `<cwd>/.openpackage/packages/<pkg>/`
- On each save, remove older WIP versions for the same workspace (per `workspaceHash`) to keep the registry clean.
- Prefer not to update the `openpackage.yml` version number; instead, keep WIP/stable details in `openpackage.index.yml` and registry metadata.
...
- The stable version is basically the current workspace version with the prerelease tag removed (the base `openpackage.yml.version`).
- **Copy the entire package** from the package root into the local registry under the stable version (same copy logic as for `save` WIP copies).
...
- No staging `openpackage.yml` mutation (per pref—version in registry metadata only).
...
- Package roots:
  - **Root package**: `<cwd>/` (files + `openpackage.yml` w/ base version)
  - **Nested package**: `<cwd>/.openpackage/packages/<pkg>/`
- Registry: `~/.openpackage/registry/<pkg>/<full-version>/` (dirs w/ files).
...
1. **Base version source/target**:
   - Where read base for `save` bump? Staging `openpackage.yml`? Registry latest stable? User arg?
   - Confirm: Staging `openpackage.yml` stays unchanged (e.g., always `1.0.0`), full WIP only in registry `index.yml`?
...
3. **Local staging after save**: Keep files at the package root (source for full copy), no special renaming needed.
...
Clarification:
1. Read base for save bump using the package root's `openpackage.yml` (either `<cwd>/openpackage.yml` for root package or `<cwd>/.openpackage/packages/<pkg>/openpackage.yml` for nested). There is a corresponding `openpackage.index.yml` file with version and files fields. For WIP builds, openpackage.yml version will always show base version, the openpackage.index.yml version will show/be updated to exact version, whether stable or wip.
...
3. Keep files at package root after save, no changes
